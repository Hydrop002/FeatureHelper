...
public class UnderwaterCavePatcher extends CavePatcher {

    public UnderwaterCavePatcher() {
        this.terrainBlocks = ImmutableSet.of(
                ...
                Blocks.WATER, Blocks.LAVA, Blocks.OBSIDIAN, Blocks.AIR, Blocks.CAVE_AIR, Blocks.PACKED_ICE
        );
    }
            ...
            for (int k = minZ; k < maxZ; ++k) {
                ...
                if (offsetRatioX * offsetRatioX + offsetRatioZ * offsetRatioZ < 1) {
                    for (int j = maxY; j > minY; --j) {
                        ...
                        if (offsetRatioY > -0.7 && offsetRatioX * offsetRatioX + offsetRatioY * offsetRatioY + offsetRatioZ * offsetRatioZ < 1 && j < this.world.getSeaLevel()) {
                            ...
                            if (this.isTargetAllowed(blockState)) {
                                if (j == 10) {
                                    if (this.rand.nextFloat() < 0.25) {
                                        ...
                                        this.world.getPendingBlockTicks().scheduleTick(blockPos, Blocks.MAGMA_BLOCK, 0);  // 产生气泡柱
                                    } else {
                                    ...
                                    for (EnumFacing facing : EnumFacing.Plane.HORIZONTAL) {
                                        ...
                                        if (blockStateNear.isAir()) {
                                            ...
                                            this.world.getPendingFluidTicks().scheduleTick(blockPos, WATER_FLUID.getFluid(), 0);  // 流动
                                            foundAir = true;
                                            break;
                                        }
                                    }
